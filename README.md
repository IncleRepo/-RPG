...
현재 플레이 화면은 캔버스 기반으로 렌더링되며, 캐릭터는 본 리그와 키프레임 클립으로 애니메이션됩니다. 플레이어와 NPC 모두 같은 리그를 공유하고, grounded 상태에서는 support surface를 기준으로 렌더 기준선을 고정한 발 IK가 적용됩니다.
...
- 지형 높이 샘플과 support surface 기준선을 사용하는 발 IK 보정
- 런타임과 동일한 리그를 미리보는 애니메이터 페이지
- 런타임과 같은 물리 코어, 가장자리 IK, 휴머노이드/발 샘플 디버그를 검증하는 Physics Lab 페이지
